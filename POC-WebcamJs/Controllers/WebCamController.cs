...
namespace POC_WebcamJs.Controllers
{
    public class WebCamController : Controller
    {
        private readonly IWebHostEnvironment _environment;
        public WebCamController(IWebHostEnvironment hostingEnvironment)
        {
        ...
        public IActionResult CaptureImage(string name)
        {
                ...
                if (files != null)
                {
                    foreach (var file in files)
                    {
                        if (file.Length > 0)
                        {
                            ...
                            var filepath = Path.Combine(_environment.WebRootPath, "CapturedImages") + $@"\{fileNameToStore}";
            ...
            {
                Debugger.Log(0, "Exception", ex.Message.ToString());
                return Json(false);
            }
        }
    }
